...
class Track < ApplicationRecord
  ...
  def recommendations
    ex_all = Track.where.not(id:)

    ex_all = ex_all.order(Arel.sql("CASE WHEN tracks.genre = #{ActiveRecord::Base.connection.quote(genre)} THEN 0 ELSE 1 END"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.tempo - #{ActiveRecord::Base.connection.quote(tempo)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.key - #{ActiveRecord::Base.connection.quote(key)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.mode - #{ActiveRecord::Base.connection.quote(mode)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.time_signature - #{ActiveRecord::Base.connection.quote(time_signature)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.acousticness - #{ActiveRecord::Base.connection.quote(acousticness)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.energy - #{ActiveRecord::Base.connection.quote(energy)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.spectral_flatness - #{ActiveRecord::Base.connection.quote(spectral_flatness)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.loudness - #{ActiveRecord::Base.connection.quote(loudness)})"))
    ex_all = ex_all.order(Arel.sql("ABS(tracks.valence - #{ActiveRecord::Base.connection.quote(valence)})"))
    ex_all
  end
end
